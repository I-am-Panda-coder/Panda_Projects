...
namespace RE
{
	...
	public:
		...
		//action should be DEFAULT_OBJECT::kActionIdle (0x35)
		...
		//panda E749A0
		uint16_t GetAll3DUpdateFlags() const {
		...
		//panda E60860
		void DoUpdate3dModel(Actor* a_actor, uint16_t flags) {
	...
	public:
		// members
		float modifiers[3];  // [0] = base value, [1] = relative value, [2] = ???
	};
	...
	public:
		...
		void EvaluatePackage(bool a_queue, bool a_resetAI)
		{
			using func_t = decltype(&Actor::EvaluatePackage);
			REL::Relocation<func_t> func{ REL::ID(1395257) };
			return func(this, a_queue, a_resetAI);
		}

		//All flags are set to true when making an actor instantly snap out of an interaction.
		...
		void WarpTo(const NiPoint3& a_point, float a_zRotation, float a_xRotation, TESObjectCELL* a_cell, TESWorldSpace* a_worldspace, bool a_resetAI, bool unk1 = false, bool unk2 = false)
		{
			using func_t = decltype(&Actor::WarpTo);
			REL::Relocation<func_t> func{ REL::ID(728864) };
			return func(this, a_point, a_zRotation, a_xRotation, a_cell, a_worldspace, a_resetAI, unk1, unk2);
		}

		void WarpToRef(const TESObjectREFR* a_dest) {
			WarpTo(a_dest->data.location, a_dest->data.angle.z, 0.0f, a_dest->parentCell, a_dest->GetWorldSpace(), false);
		}
